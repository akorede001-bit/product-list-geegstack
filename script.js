...
// fetch all product data on page load
...
.then(response => {
  ...
  addToCartImg.setAttribute("src", "./assets/images/icon-add-to-cart.svg");
    ...
    decrementSVG.classList.add("decrement-path");
    ...
    incrementSVG.setAttribute("fill", "0  0 10 10");
...
const displayCartItems = () => {
  ...
  const totalItemsSpan = document.querySelector(".carts > h1 span");
 ...
 totalItemsSpan.innerHTML = totalItems;
...
window.onclick = (e) => {
  ...
  if (e.target.className === "add-to-cart") {
    ...
    totalPrice: product.price, thumbnail: product.image.thumbnail };
  ...
  if (e.target.classList.contains ("remove-icon")|| e.target.classList.contains("remove-svg-path")) {
    const productsArticle = (e.target.tagName ==="path")? e.target.parentElement.parentElement : e.
    target.parentElement;
    const productName = productsArticle.querySelector(".cart-item-name").innerHTML;
...
if (e.target.classList.contains ("increment-svg")|| e.target.classList.contains("increment-path")) {
  const parentDiv = (e.target.tagName ==="path")? e.target.parentElement.parentElement : e.target.
 parentElement; console.log(parentDiv.nextElementSibling);
  const productName = parentDiv.nextElementSibling.nextElementSibling.querySelector(".product-name").
  innerText;
...
if (e.target.classList.contains ("decrement-svg")|| e.target.classList.contains("decrement-path")) {
 ...
 parentElement; console.log(parentDiv.nextElementSibling);
  ...
  if (product.quantity === 1) {
      const otherProducts = cartItems.filter(cart.name !== productName);
} else{
    product.quantity -=1;
}
...
confirmOrderBtn.onclick = function() {
  ...
  for (let i = 0; i < cartItems.length; i++) {
    ...
    quantityPar.append(quantitySpan, priceTextNode);
    div.append(productPar, quantityPar);
    parentDiv.append(img, div);
    pricePar.innerHTML = "$" + (item.totalPrice * item.quantity).toFixed(2);
